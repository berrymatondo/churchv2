...
export default function ContinentsPage() {
  ...
  const [editingContinent, setEditingContinent] = useState<Continent | null>(
    null
  );
  ...
  const fetchData = async () => {
    try {
      const [continentsResponse, paysResponse] = await Promise.all([
        fetch("/api/continents"),
        fetch("/api/pays"),
      ]);

      const [continentsResult, paysResult] = await Promise.all([
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingContinent
        ? `/api/continents/${editingContinent.id}`
        : "/api/continents";
      ...
      if (result.success) {
        toast({
          title: "Succès",
          description: editingContinent
            ? "Continent modifié avec succès"
            : "Continent créé avec succès",
        });
        setDialogOpen(false);
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 py-8">
        <EntityBreadcrumb
          items={[{ label: "Continents", current: true }]}
          className="mb-6"
        />
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <CardDescription>
                      Gérez les continents de votre organisation
                    </CardDescription>
                  </div>
            ...
            {selectedContinent ? (
              <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="text-blue-600 font-medium">
                          {
                            pays.filter(
                              (p) => p.continentId === selectedContinent.id
                            ).length
                          }
                        </span>
                      </div>
                ...
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      Cliquez sur un pays pour le gérer
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
              ...
              <Card>
                <CardHeader>
                  <CardTitle>Détails du Continent</CardTitle>
                  <CardDescription>
                    Cliquez sur un continent dans le tableau pour voir ses
                    détails et pays
                  </CardDescription>
                </CardHeader>
              </Card>
        ...
        <Dialog open={mobileModalOpen} onOpenChange={setMobileModalOpen}>
          <DialogContent className="w-[95vw] max-w-lg max-h-[85vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>
                Informations et pays du continent sélectionné
              </DialogDescription>
            </DialogHeader>
            {selectedContinent && (
              <div className="space-y-4">
          ...
          <DialogContent>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <DialogFooter>
                ...
                <Button type="submit">
                  {editingContinent ? "Modifier" : "Ajouter"}
                </Button>
              </DialogFooter>
            </form>
          </DialogContent>
